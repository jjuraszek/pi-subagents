...
pi-gauntlet copy - keep them in sync.
...
pins (`npm:pi-cohort@<old>`) are bumped by `--apply`; git-tag pins and stale
`pi-subagents` names are reported for manual migration, never auto-rewritten.
...
`pi-cohort` must be registered once as a **trusted publisher** on npmjs.com:
Settings -> Trusted Publishing -> GitHub Actions publisher for repo
`jjuraszek/pi-cohort`, workflow `release.yml`. Until it exists the publish step
